...
local Library = { } do
	...
	Library.IconGrid = function(Self, Params)
		Params = Params or { }

		local Section = Self
		local Columns = math.max(1, math.floor(Params.Columns or 2))
		local Rows = math.max(1, math.floor(Params.Rows or 3))
		local Count = Columns * Rows
		local Gap = math.max(0, math.floor(Params.Gap or 8))
		local CardHeight = math.max(72, math.floor(Params.CardHeight or 108))
		local Padding = math.max(0, math.floor(Params.Padding or 15))
		local GridWidth = Section.Width - (Padding * 2)
		local CardWidth = math.floor((GridWidth - (Gap * (Columns - 1))) / Columns)
		local RowHeight = (Rows * CardHeight) + (Gap * (Rows - 1)) + 8

		local Grid = {
			Name = Params.Name or "Icon Grid",
		...
		local function ClearViewport(Card)
			for _, Child in Card.Viewport.Instance:GetChildren() do
				if Child ~= Card.Camera.Instance and Child ~= Card.World.Instance then
					Child:Destroy()
				end
			end
			ClearWorld(Card.World.Instance)
			Card.Viewport.Instance.CurrentCamera = Card.Camera.Instance
		end
		...
		local function PrepareViewport(Card, Model)
			ClearViewport(Card)
			...
			Clone.Parent = Card.World.Instance

			local Size
			local Center = Vector3.zero
			if Clone:IsA("Model") then
				pcall(function()
					Clone:PivotTo(CFrame.Angles(0, math.rad(-24), 0))
					local BoxCF, BoxSize = Clone:GetBoundingBox()
					Clone:PivotTo(Clone:GetPivot() + (Vector3.zero - BoxCF.Position))
					Size = BoxSize
				end)
			elseif Clone:IsA("BasePart") then
				Clone.CFrame = CFrame.Angles(0, math.rad(-24), 0)
				Size = Clone.Size
			else
			...
			local Fov = 35
			local Distance = (MaxDim * 0.5) / math.tan(math.rad(Fov * 0.5)) * 1.45
			Card.Camera.Instance.FieldOfView = Fov
			Card.Camera.Instance.CFrame = CFrame.lookAt(
				Center + Vector3.new(MaxDim * 0.12, Size.Y * 0.08, Distance),
				Center
			)
			Card.Viewport.Instance.CurrentCamera = Card.Camera.Instance
			Card.Viewport.Instance.Visible = true
		...
		for Index = 1, Count do
			...
			local Column = CellIndex % Columns
			local Line = math.floor(CellIndex / Columns)
			...
			Card.Stroke = Library:Create("UIStroke", {
			...
			Card.Image = MakeImage({
				...
				Raw = Color3.new(1, 1, 1),
				Fit = true,
				Icon = Params.FallbackIcon or "package",
				Z = 6
			})

			Card.Viewport = Library:Create("ViewportFrame", {
				Parent = Card.Frame.Instance,
				Name = "\0",
				BackgroundTransparency = 1,
			...
			Card.Camera = Library:Create("Camera", {
				Parent = Card.Viewport.Instance,
				Name = "\0"
			})
			Card.World = Library:Create("WorldModel", {
				Parent = Card.Viewport.Instance,
				Name = "\0"
			})
			Card.Viewport.Instance.CurrentCamera = Card.Camera.Instance
			...
			Card.Subtitle = MakeText({
				Parent = Card.Frame.Instance,
				Text = "",
				TextSize = 11,
				Pos = UDim2.new(0, 7, 1, -16),
			...
			Card.Hit:Connect("MouseEnter", function()
				if not Card.Data then return end
				Card.Frame:Tween({ BackgroundColor3 = Library.Theme.Hover })
			end)
			Card.Hit:Connect("MouseLeave", function()
				if not Card.Data then return end
				local Selected = IsSelected(Card.Data)
				Card.Frame:Tween({ BackgroundColor3 = Selected and Library.Theme.Light or Library.Theme.Element })
			end)
